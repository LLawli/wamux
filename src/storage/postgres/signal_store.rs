//! `SignalStore` over Postgres. All key material is stored as raw bytes,
//! matching the sqlite reference (no transform).
...
impl SignalStore for PgBackend {
    ...
    async fn load_identity(&self, address: &str) -> Result<Option<[u8; 32]>> {
        let row: Option<Vec<u8>> =
            sqlx::query_scalar("SELECT key FROM identities WHERE address = $1 AND device_id = $2")
                .bind(address)
                .bind(self.device_id)
    ...
    async fn get_session(&self, address: &str) -> Result<Option<Bytes>> {
        let row: Option<Vec<u8>> =
            sqlx::query_scalar("SELECT record FROM sessions WHERE address = $1 AND device_id = $2")
                .bind(address)
                .bind(self.device_id)
                .fetch_optional(&self.pool)
                .await
    ...
    async fn get_max_prekey_id(&self) -> Result<u32> {
        let max: i32 =
            sqlx::query_scalar("SELECT COALESCE(MAX(id), 0) FROM prekeys WHERE device_id = $1")
                .bind(self.device_id)
                .fetch_one(&self.pool)
                .await
                .map_err(db)?;
    ...
    async fn load_all_signed_prekeys(&self) -> Result<Vec<(u32, Vec<u8>)>> {
        let rows: Vec<(i32, Vec<u8>)> =
            sqlx::query_as("SELECT id, record FROM signed_prekeys WHERE device_id = $1")
                .bind(self.device_id)
                .fetch_all(&self.pool)
                .await
                .map_err(db)?;
