//! `ProtocolStore` over SQLite. Mirrors `postgres/protocol_store.rs` statement
//! for statement; only the dialect differs (`?` placeholders): per-device
//! sender-key tracking, LID-PN mappings, base keys, device registry, tc tokens,
//! and the sent-message cache.
...
impl ProtocolStore for SqliteBackend {
    ...
    async fn get_sender_key_devices(&self, group_jid: &str) -> Result<Vec<(String, bool)>> {
        let rows: Vec<(String, i32)> = sqlx::query_as(
            "SELECT device_jid, has_key FROM sender_key_devices WHERE group_jid = ? AND device_id = ?",
        )
        .bind(group_jid)
        .bind(self.device_id)
        .fetch_all(&self.pool)
    ...
    async fn clear_sender_key_devices(&self, group_jid: &str) -> Result<()> {
        sqlx::query("DELETE FROM sender_key_devices WHERE group_jid = ? AND device_id = ?")
            .bind(group_jid)
            .bind(self.device_id)
            .execute(&self.pool)
            .await
    ...
    async fn delete_sender_key_device_rows(&self, device_jids: &[&str]) -> Result<()> {
        ...
        for device_jid in device_jids {
            sqlx::query("DELETE FROM sender_key_devices WHERE device_jid = ? AND device_id = ?")
                .bind(device_jid)
                .bind(self.device_id)
                .execute(&mut *tx)
                .await
    ...
    async fn clear_all_sender_key_devices(&self) -> Result<()> {
        sqlx::query("DELETE FROM sender_key_devices WHERE device_id = ?")
            .bind(self.device_id)
            .execute(&self.pool)
            .await
            .map_err(db)?;
    ...
    async fn get_lid_mapping(&self, lid: &str) -> Result<Option<LidPnMappingEntry>> {
        let row: Option<(String, String, i64, String, i64)> = sqlx::query_as(
            "SELECT lid, phone_number, created_at, learning_source, updated_at
             FROM lid_pn_mapping WHERE lid = ? AND device_id = ?",
        )
        .bind(lid)
        .bind(self.device_id)
    ...
    async fn get_pn_mapping(&self, phone: &str) -> Result<Option<LidPnMappingEntry>> {
        let row: Option<(String, String, i64, String, i64)> = sqlx::query_as(
            "SELECT lid, phone_number, created_at, learning_source, updated_at
             FROM lid_pn_mapping WHERE phone_number = ? AND device_id = ?
    ...
    async fn delete_base_key(&self, address: &str, message_id: &str) -> Result<()> {
        sqlx::query("DELETE FROM base_keys WHERE address = ? AND message_id = ? AND device_id = ?")
            .bind(address)
            .bind(message_id)
            .bind(self.device_id)
            .execute(&self.pool)
    ...
    async fn get_devices(&self, user: &str) -> Result<Option<DeviceListRecord>> {
        let row: Option<(String, String, i64, Option<String>, Option<i32>)> = sqlx::query_as(
            "SELECT user_id, devices_json, timestamp, phash, raw_id
             FROM device_registry WHERE user_id = ? AND device_id = ?",
        )
        .bind(user)
        .bind(self.device_id)
        .fetch_optional(&self.pool)
    ...
    async fn delete_devices(&self, user: &str) -> Result<()> {
        sqlx::query("DELETE FROM device_registry WHERE user_id = ? AND device_id = ?")
            .bind(user)
            .bind(self.device_id)
            .execute(&self.pool)
            .await
    ...
    async fn get_tc_token(&self, jid: &str) -> Result<Option<TcTokenEntry>> {
        let row: Option<(Vec<u8>, i64, Option<i64>)> = sqlx::query_as(
            "SELECT token, token_timestamp, sender_timestamp
             FROM tc_tokens WHERE jid = ? AND device_id = ?",
        )
        .bind(jid)
        .bind(self.device_id)
        .fetch_optional(&self.pool)
    ...
    async fn delete_tc_token(&self, jid: &str) -> Result<()> {
        sqlx::query("DELETE FROM tc_tokens WHERE jid = ? AND device_id = ?")
            .bind(jid)
            .bind(self.device_id)
            .execute(&self.pool)
            .await
    ...
    async fn get_all_tc_token_jids(&self) -> Result<Vec<String>> {
        let rows: Vec<String> = sqlx::query_scalar("SELECT jid FROM tc_tokens WHERE device_id = ?")
            .bind(self.device_id)
            .fetch_all(&self.pool)
            .await
            .map_err(db)?;
    ...
    async fn delete_expired_tc_tokens(&self, token_cutoff: i64, sender_cutoff: i64) -> Result<u32> {
        let res = sqlx::query(
            "DELETE FROM tc_tokens
             WHERE (length(token) = 0 OR token_timestamp < ?)
               AND (sender_timestamp IS NULL OR sender_timestamp < ?)
               AND device_id = ?",
        )
        .bind(token_cutoff)
        .bind(sender_cutoff)
    ...
    async fn take_sent_message(&self, chat_jid: &str, message_id: &str) -> Result<Option<Vec<u8>>> {
        ...
        let payload: Option<Vec<u8>> = sqlx::query_scalar(
            "SELECT payload FROM sent_messages
             WHERE chat_jid = ? AND message_id = ? AND device_id = ?",
        )
        .bind(chat_jid)
        .bind(message_id)
        .bind(self.device_id)
    ...
    async fn delete_expired_sent_messages(&self, cutoff_timestamp: i64) -> Result<u32> {
        let res = sqlx::query("DELETE FROM sent_messages WHERE created_at < ? AND device_id = ?")
